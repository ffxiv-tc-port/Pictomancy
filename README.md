...
* `BackbufferAlpha` is the only UI mask; it does not work with 3D resolution scaling.
    * `BackbufferAlpha` will fallback to `None` when it detects 3D resolution scaling.
